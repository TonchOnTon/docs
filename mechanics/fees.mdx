...
The fee accrues in the curve contract's `accumulated_fees` state field. It can be swept to the protocol treasury at any time via `op::sweep_fees` — a permissionless opcode any address can trigger as long as enough gas is attached. tonch's keeper triggers it operationally so deployers and traders never have to think about it.

For a curve that fills with moderate volume, the protocol collects roughly 1% of the cumulative TON traded — for a 1,000 TON curve, that's about 10 TON, plus any extra volume from sells (which also pay the fee).
...
| Sweep fees (anyone, post-graduation) | 0.05 TON |
...
A deployer launches a token, dev-buys 5 TON, and the token graduates after total cumulative trading volume of 1,200 TON (some buying pressure followed by some selling).

- **Deployer pays:** 3 TON (launch) + 5 TON (dev-buy) + ~0.7 TON (gas) = **~8.7 TON out**.
- **Deployer keeps:** the Jettons their 5 TON dev-buy bought, now tradeable on DeDust.

Net: the deployer spends ~8.7 TON in TON for whatever Jetton position the dev-buy bought (a free option on the post-graduation price). They do not collect any fee revenue.

The protocol's revenue from this token: 3 TON launch fee + ~12 TON curve fees + 100 TON graduation cut = **~115 TON**.
